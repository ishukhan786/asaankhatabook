...
export async function exportStatementPDF(
  ...
  const money            = (n: number | string | null | undefined) => formatMoney(n, displayCurrency);
  ...
  const drawHeader = () => {
    ...
    doc.setFontSize(7.5);
    doc.setTextColor(180, 198, 230);
    doc.text(`Generated: ${generatedAt}`, W - 12, 26, { align: "right" });
  };
  ...
  const drawTableLabel = () => {
    ...
    doc.setFontSize(7.5);
    doc.setTextColor(...rgb(C.accent));
    doc.text("VOUCHER HISTORY", 16, TABLE_LABEL_Y + 5.2);
    doc.setFont("helvetica", "normal");
  ...
  autoTable(doc, {
    ...
    didParseCell: (data) => {
      ...
      if (data.column.index === 3 && data.cell.raw !== "—") {
        data.cell.styles.textColor = C.ink;
        data.cell.styles.fillColor = [255, 245, 248];
      }
    ...
    tableLineWidth: 0.12,
    // New page — redraw border (header, info card & summary are ONLY on Page 1)
