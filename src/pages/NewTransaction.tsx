...
const transactionTypeOptions = [
  { value: "general", label: "General Voucher", prefix: "VCH" },
  { value: "payment", label: "Payment Voucher", prefix: "PAY" },
  { value: "receipt", label: "Receipt Voucher", prefix: "RCP" },
  { value: "transfer", label: "Transfer Voucher", prefix: "TRF" },
  { value: "expense", label: "Expense Voucher", prefix: "EXP" },
  { value: "journal", label: "Journal Voucher", prefix: "JRN" },
] as const;
...
export default function NewTransaction() {
  ...
  const yearStr = form.txn_date ? form.txn_date.slice(0, 4) : new Date().getFullYear().toString();
  const txnCodePreview = `VCH-${yearStr}-00001`;
  ...
  const sendWhatsApp = (t: { txn_date: string; details?: string }, acc: AccountShort) => {
    ...
    const type = Number(form.credit) > 0 ? "Jama (Credit)" : "Banam (Debit)";
