...
import { Search, ArrowDownLeft, Phone, MapPin, Building2, MessageSquare, Printer, ExternalLink, Download } from "lucide-react";
...
export default function Receivables() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const processed: AccountBalance[] = (accs ?? [])
        ...
        .filter(a => a.balance < 0) // Receivables
        .sort((a, b) => a.balance - b.balance);

      setAccounts(processed);
    } catch (err) {
      logger.error("Receivables load error:", err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const sendWhatsApp = (e: React.MouseEvent, a: AccountBalance) => {
    e.stopPropagation();
    if (!a.mobile) return;
    const msg = `*Assalam-o-Alaikum ${a.name}!*\n\n*Aasaan Khatabook Receivable Reminder*\n---------------------------\n*Account:* ${a.name}\n*Account No:* ${a.account_no}\n*Receivable Amount:* ${formatMoney(Math.abs(a.balance), a.currency)}\n---------------------------\nShukriya!`;
  ...
  const handleExportCSV = () => {
    const data = filtered.map(a => ({
      ...
      "Currency": a.currency,
      "Receivable Amount": Math.abs(a.balance),
    }));
    exportToCSV(data, `Receivables_Report_${new Date().toISOString().slice(0, 10)}`);
  };
    ...
    <>
      ...
      <div id="print-receivables-wrapper" style={{ display: "none" }}>
        <div style={{ fontFamily: "'Inter', sans-serif", background: "#ffffff", color: "#0f172a", fontSize: "11px", padding: "10px" }}>
          ...
          <div style={{ height: "6px", background: "linear-gradient(90deg, #dc2626 0%, #ef4444 100%)", borderRadius: "3px" }} />
          ...
          <div style={{ padding: "16px 0 12px", borderBottom: "2px solid #e2e8f0", display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
            <div>
              <div style={{ fontSize: "22px", fontWeight: "900", color: "#991b1b", letterSpacing: "-0.5px" }}>
                {profile?.business_name || "AsaanKhata"}
              </div>
              <div style={{ fontSize: "13px", fontWeight: "800", color: "#dc2626", marginTop: "3px", textTransform: "uppercase", letterSpacing: "0.5px" }}>
            ...
            <div style={{ textAlign: "right", background: "#fef2f2", border: "1px solid #fca5a5", padding: "8px 12px", borderRadius: "8px" }}>
              <div style={{ fontSize: "10px", fontWeight: "700", color: "#dc2626" }}>REPORT METRICS</div>
          ...
          <div style={{ margin: "14px 0", background: "#fef2f2", border: "1.5px solid #fca5a5", borderRadius: "12px", padding: "14px 18px" }}>
            <div style={{ fontSize: "10px", fontWeight: "800", textTransform: "uppercase", color: "#dc2626", letterSpacing: "0.5px", marginBottom: "10px", display: "flex", justifyContent: "space-between" }}>
              <span>TOTAL OUTSTANDING RECEIVABLES SUMMARY ({filtered.length} ACCOUNTS)</span>
              <span>STATUS: ACTIVE ASSET</span>
            </div>
            
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: "12px" }}>
              {totals.map(([cur, amount]) => (
                <div key={cur} style={{ background: "#ffffff", border: "1px solid #fca5a5", borderRadius: "8px", padding: "10px 14px" }}>
                  <div style={{ fontSize: "9.5px", fontWeight: "700", color: "#dc2626", textTransform: "uppercase" }}>
                    {cur === "PKR" ? "🇵🇰 PKR Total" : cur === "AED" ? "🇦🇪 AED Total" : cur === "USD" ? "🇺🇸 USD Total" : `${cur} Total`}
                  </div>
                  <div style={{ fontSize: "17px", fontWeight: "900", color: "#991b1b", fontFamily: "monospace", marginTop: "3px" }}>
                    {formatMoney(amount, cur)}
                  </div>
                </div>
              ))}
              {totals.length === 0 && (
                <div style={{ background: "#ffffff", border: "1px solid #fca5a5", borderRadius: "8px", padding: "10px 14px" }}>
                  <div style={{ fontSize: "9.5px", fontWeight: "700", color: "#dc2626" }}>TOTAL BALANCE</div>
                  <div style={{ fontSize: "17px", fontWeight: "900", color: "#991b1b", fontFamily: "monospace", marginTop: "3px" }}>0.00</div>
                </div>
              )}
            </div>
          </div>
              ...
              {filtered.length === 0 ? (
                <tr>
                  <td colSpan={6} style={{ padding: "20px", textAlign: "center", color: "#64748b" }}>
                    No receivable records found for print.
                  </td>
                </tr>
              ) : (
                filtered.map((a, i) => (
                  <tr key={a.id} style={{ background: i % 2 === 0 ? "#ffffff" : "#fef2f2", borderBottom: "1px solid #e2e8f0" }}>
                    ...
                    <td style={{ padding: "7px 10px", textAlign: "right", fontWeight: "900", color: "#dc2626", fontFamily: "monospace", fontSize: "11.5px" }}>
          ...
          <div style={{ marginTop: "40px", paddingTop: "15px", borderTop: "1px solid #e2e8f0", display: "flex", justifyContent: "space-between", alignItems: "flex-end" }}>
            <div style={{ width: "200px", textAlign: "center" }}>
              ...
              <div style={{ fontSize: "9px", fontWeight: "700", color: "#64748b" }}>PREPARED BY (ACCOUNTANT)</div>
            </div>
            <div style={{ width: "200px", textAlign: "center" }}>
              <div style={{ borderBottom: "1.5px border #94a3b8", height: "30px", marginBottom: "4px" }} />
              <div style={{ fontSize: "9px", fontWeight: "700", color: "#64748b" }}>APPROVED BY / STAMP</div>
            </div>
          </div>
      ...
      <div className="screen-ui p-4 md:p-8 max-w-[1600px] mx-auto space-y-6">
        <PageHeader
          ...
          actions={
            <div className="flex items-center gap-3 flex-wrap">
              ...
              <Button onClick={() => window.print()} variant="outline" className="h-11 px-4 gap-2 border-2 rounded-xl font-semibold hover:bg-red-600 hover:text-white">
        ...
        {/* Summary KPI Card */}
        <Card className="relative overflow-hidden border-2 border-red-200 dark:border-red-900/50 bg-gradient-to-br from-red-50/60 to-white dark:from-red-950/20 dark:to-card p-6 rounded-2xl shadow-md">
          <div className="absolute left-0 top-0 bottom-0 w-2 bg-red-500" />
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div className="flex items-center gap-5">
              <div className="w-16 h-16 rounded-2xl bg-red-500 text-white flex items-center justify-center shadow-lg shadow-red-500/25 shrink-0">
                <ArrowDownLeft className="w-8 h-8" />
              </div>
              <div>
                <div className="text-xs font-bold uppercase tracking-widest text-red-600 dark:text-red-400 mb-1 flex items-center gap-2">
                  <span>{t("TotalReceivable")}</span>
                  <span className="bg-red-100 dark:bg-red-900/40 text-red-700 dark:text-red-300 px-2 py-0.5 rounded-full text-[10px]">{filtered.length} {t("TotalAccounts")}</span>
                </div>
                <div className="flex flex-wrap items-center gap-3 mt-2">
                  {totals.map(([cur, amount]) => (
                    <div key={cur} className="flex items-center gap-2 bg-red-100/80 dark:bg-red-950/60 border border-red-300 dark:border-red-800/60 px-3.5 py-1.5 rounded-xl shadow-xs">
                      <span className="text-xs font-extrabold uppercase text-red-800 dark:text-red-300">
                        {cur === "PKR" ? "🇵🇰 PKR" : cur === "AED" ? "🇦🇪 AED" : cur === "USD" ? "🇺🇸 USD" : cur}
                      </span>
                      <span className="font-black text-lg num text-red-700 dark:text-red-300">
                        {formatMoney(amount, cur)}
                      </span>
                    </div>
                  ))}
                  {totals.length === 0 && <span className="text-2xl font-black text-red-600 dark:text-red-400 num">0</span>}
                </div>
              </div>
            </div>
            ...
            <div className="relative max-w-md w-full md:w-80">
              ...
              <Input 
                ...
                className="pl-9 h-11 bg-background/60 border-red-200 dark:border-red-900/40 rounded-xl"
              />
            </div>
          </div>
        </Card>
        ...
        <Card className="rounded-2xl border-2 border-red-200 dark:border-red-900/40 overflow-hidden shadow-sm">
          <div className="overflow-x-auto">
            <table className="w-full text-sm whitespace-nowrap">
              <thead>
                <tr className="bg-red-50/70 dark:bg-red-950/30 border-b-2 border-red-200 dark:border-red-900/40 text-red-700 dark:text-red-400 text-xs uppercase tracking-wider font-bold">
                  ...
                  <th className="px-4 py-3.5 text-right">{t("Receivables")}</th>
                    ...
                    <tr key={a.id} className="hover:bg-muted/30 transition-colors cursor-pointer group" onClick={() => navigate(`/accounts/${a.id}`)}>
                      ...
                      <td className="px-4 py-3.5">
                        <div className="flex items-center gap-3">
                          <div className="w-9 h-9 rounded-xl bg-red-100 text-red-700 dark:bg-red-950 dark:text-red-400 flex items-center justify-center text-xs font-bold shrink-0">
                      ...
                      <td className="px-4 py-3.5 text-right">
                        <div className="font-black text-base num text-red-600 dark:text-red-400 tracking-tight">
                          {formatMoney(Math.abs(a.balance), a.currency)}
                        </div>
                      </td>
